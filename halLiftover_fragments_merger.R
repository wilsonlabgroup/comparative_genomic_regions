...
# After merging, filter the merged fragmenets by length relative to original query peaks (controled by --max_perc and --min_perc)
...
option_list <- list(
  ...
  make_option("--max_perc",default = 125, help = "Maximum percentage of clustered regions/peaks width, Default 125 (%)"),
  make_option("--min_perc", default = 75, help = "Minimum percentage of clustered regions/peaks width, Default 25 (%)"),
...
# if min/max length parameters is given, use the fixed parameters instead 
if(length(intersect(c("max_len","min_len"),names(args))) == 2){
  filter_by <- "length"
...
if(length(intersect(c("min_perc","max_perc"),names(args))) == 2){
  filter_by <- "frac"
  MIN_PERC <- args$min_perc
  MAX_PERC <- args$max_perc
  print(paste0("Filter merged regions by relative fraction of peaks, between ", MIN_PERC, " and ", MAX_PERC))
}
...
targets_merged_df$perc <- with(targets_merged_df, 100*(width/peak_width))
...
if(filter_by == "frac"){
  targets_merged_df_fil <- subset(targets_merged_df, perc >= MIN_PERC & perc <= MAX_PERC)
} else{
  targets_merged_df_fil <- subset(targets_merged_df, peak_width >= MIN_LEN & peak_width <= MAX_PERC)
}
